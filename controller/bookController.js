...
const getBook = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ success: true, message: "all book fetched successfully..!", books });
    } catch (error) {
        next(new HttpError(error.message, 500));
    }
}
const updateBook = async (req, res, next) => {
    try {
        ...
        const allowedField = ["title", "author", "category", "totalCopies", "availableCopies"];
        ...
        res.status(200).json({ success: true, message: "data updated successfully", book });
    } catch (error) {
        next(new HttpError(error.message, 500));
    }
}
const deleteBook = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ success: true, message: "book delete successfully" });
    } catch (error) {
        next(new HttpError(error.message, 500));
    }
}
export default { getBook, updateBook, deleteBook, createBook }; 